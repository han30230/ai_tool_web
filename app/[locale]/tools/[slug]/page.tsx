...
export async function generateMetadata({
  ...
  const canonical = `${getBaseUrl()}/${locale}/tools/${slug}`;
...
export default async function ToolDetailPage({
  ...
  const toolPageUrl = `${baseUrl}/${locale}/tools/${tool.slug}`;
    ...
    mainEntity: [
      {
        ...
        acceptedAnswer: {
          "@type": "Answer",
          text:
            (tool.short_description || tool.description || "").split(/\n\n+/)[0]?.trim() ||
            `${tool.name} 소개 페이지입니다.`,
        },
      },
      {
  ...
  const jsonLdBreadcrumb = {
    ...
    itemListElement: [
      ...
      { "@type": "ListItem", position: 2, name: "AI 서비스 모음", item: `${baseUrl}/${locale}/tools` },
      ...(category
        ? [
            {
              ...
              position: 3,
              name: category.name,
              item: `${baseUrl}/${locale}/categories/${tool.category}`,
            },
          ]
        : []),
      {
        ...
        name: tool.name,
        item: `${baseUrl}/${locale}/tools/${tool.slug}`,
      },
    ],
  };
        ...
        <div className="flex flex-wrap items-start gap-4">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="mt-4 flex flex-wrap items-center gap-4">
              <ShareButtons url={toolUrl} title={`${tool.name} | ${tCommon("siteName")}`} description={shortDesc} />
            </div>
        ...
        <div className="space-y-6 sm:space-y-8 lg:col-span-2">
          ...
          <section className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm">
            ...
            {pricingPlans.length > 0 ? (
              ...
              <div className="mt-3 rounded-lg border border-slate-200 bg-slate-50 p-4 text-sm text-slate-600">
                현재 이 툴의 <span className="font-semibold">플랜별 가격 정보가 아직 정리되지 않았습니다</span>. 아래 버튼에서
                공식 가격 페이지를 확인해 주세요.
              </div>
            )}
            <a
          ...
          {relatedArticles.length > 0 && (
            <section className="rounded-xl border border-slate-200 bg-white p-4 sm:p-5 shadow-sm">
              ...
              <div className="mt-3 h-px bg-slate-200" />
              <p className="mt-3 text-sm text-slate-600">{tool.name}와(과) 함께 보면 좋은 글을 모았습니다.</p>
              <ul className="mt-3 space-y-2">
