...
## 1-추가. 툴 가격 정보를 “반자동”으로 최신화하기 (상위 50개)

툴 상세 페이지에서 가격을 “공식 홈페이지 확인”으로만 안내하면 신뢰도가 떨어지기 때문에, 가격 페이지를 주기적으로 확인해 `data/tools.json`의 가격 필드를 갱신하는 스크립트를 추가했습니다.

- **대상**: 상위 50개 툴(기본 정렬: `featured` 우선 + `id` 최신)
- **갱신되는 필드**:
  - `pricing_url`: 가능한 경우 실제 가격 페이지로 보정
  - `pricing_last_updated_at`: 갱신일(YYYY-MM-DD)
  - `pricing_plans`: (가능하면) 플랜/가격 추출 결과
  - `pricing_note`: “자동 수집” 여부 및 주의 문구

실행:

```bash
npm run pricing:update:top50
```

주의:

- 모든 사이트가 자동 추출에 친화적이지 않습니다(로그인/지역/AB 테스트/동적 렌더링 등). 이 경우 `pricing_plans`는 비어 있을 수 있습니다.
- 자동 추출은 **정확한 계약/청구 금액 보장**이 아니라 “가격표 요약” 목적입니다. 상세는 `pricing_url` 출처를 함께 제공합니다.
